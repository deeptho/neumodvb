...
/** Renders SVG in internal image and returns a buffer suitable for opengl */
unsigned char* wxSVGDocument::RenderGetRef(int width, int height, const wxSVGRect* rect, bool preserveAspectRatio, bool alpha) {
	if (!GetRootElement())
		return nullptr;
	...
	if (GetRootElement()->GetWidth().GetAnimVal() > 0 && GetRootElement()->GetHeight().GetAnimVal() > 0) {
		if (preserveAspectRatio) {
			...
			if (m_scale > height / GetRootElement()->GetHeight().GetAnimVal())
				m_scale = height / GetRootElement()->GetHeight().GetAnimVal();
			int xmargin = width - m_scale*  GetRootElement()->GetWidth().GetAnimVal();
			int ymargin = height - m_scale*  GetRootElement()->GetHeight().GetAnimVal();
			m_screenCTM = m_screenCTM.Translate(+xmargin/2, ymargin/2);
			m_screenCTM = m_screenCTM.Scale(m_scale);
			//m_screenCTM = m_screenCTM.Translate(+xmargin/(2*m_scale), ymargin/(2*m_scale));
#if 0
			width = (int) (m_scale * GetRootElement()->GetWidth().GetAnimVal());
			height = (int) (m_scale * GetRootElement()->GetHeight().GetAnimVal());
#endif
	...
	if (!alpha)
		m_canvas->Clear(*wxWHITE);
	GetCanvas()->RenderElement(GetRootElement(), rect, &m_screenCTM, &GetRootElement()->GetStyle(), NULL, NULL, NULL);
