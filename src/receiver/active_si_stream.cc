...
/*
	subscription_id: if subscription_id==subscription_id_t::NONE, then do not register
	as being scanned
 */
void active_si_stream_t::add_si_subscription(const chdb::any_mux_t& mux,
																									devdb::scan_target_t scan_target,
																									subscription_id_t subscription_id)
{
#ifndef NDEBUG
	//old mux or service needs to be removed first
	assert (!this->subscription_exists(subscription_id));
#endif
	...
	assert(!this->subscription_exists(subscription_id));
	auto is_first = this->subscriptions.size() == 0;
	this->subscriptions[subscription_id] = mux;
	auto& scan_id = mux_common_ptr(mux)->scan_id;
	bool our_scan = subscription_id != subscription_id_t::NONE && scanner_t::is_our_scan(scan_id);
	if(our_scan) {
		if(is_first) {
			this->activate_scan_(this->dbmux, scan_id);
			active_adapter().on_stream_mux_change(this->dbmux);
		}
		this->activate_scan(subscription_id, scan_id);
	}
}

/*
	returns true if the last subscription was removed
 */
bool active_si_stream_t::remove_si_subscription(subscription_id_t subscription_id)
{
	auto it = this->subscriptions.find(subscription_id);
	auto& mux = it->second;
	this->finalize_scan_for_mux_(mux, false /*is_main_mux*/);
	this->subscriptions.erase(subscription_id);
...
void active_si_stream_t::activate_scan(subscription_id_t subscription_id,
	...
	dtdebugf("activate_scan mux={}", this->dbmux);
	using namespace chdb;
	assert((int)subscription_id >= 0);

	if(scan_state.scans_in_progress.contains({scan_id, subscription_id})) {
		dtdebugf("scan already activated: {}\n", (int)subscription_id);
		return;
	}
...
void active_si_stream_t::finalize_scan_for_mux_(chdb::any_mux_t& mux_, bool is_main_mux)
{
	/*
		this->dbmux is a copy of one of the muxes, and must also be updated
	*/
	...
	*c = *chdb::mux_common_ptr(this->dbmux); //copy over key_src and such
	...
	if(is_main_mux) {
		...
		dttime_init();
		assert(&mux_ == & this->dbmux);
		active_adapter().on_stream_mux_change(this->dbmux);//needed to ensure that mux_common changes are not overwritten in save_pmts
		dttime(200);
	}
	if(nosave) {
		return;
	...
	if(lock_state.is_dvb  &&
		...
		auto ret=this->update_mux(wtxn, mux, now, is_main_mux /*is_reader_mux*/, true /*is_tuned_freq*/,
										 false /*from_sdt*/, false /*from_pat*/, preserve);
		if(ret) {
#ifndef NDEBUG
...
/*
	...
	a scan is considered final, i.e., when
	 ...
	 -this specfic requested stream no longer exists on a multi-stream, because the
	  driver tuned another one, despite our request for a specific ISI
...
void active_si_stream_t::check_scan_mux_end()
{
	...
	dtdebugf("calling on_scan_mux_end dbfe={} mux={} scan_result={}  num_subs={}",
					 dbfe, mux, mux_common_ptr(mux)->scan_result, scans_in_progress.size());
...
/*
	reset the active_si_stream gracefully, notifying scanner
	and then stopping the associated streams
 */
void active_si_stream_t::end_si() {
	...
	/* TODO:  check that this also closes any open pid.
		 There should not be any left, unless a stream changes from ts to non-ts
...
/*
	reset the active_si_stream fully but do not notify scanner
 */
																 ...
																 bool is_active_mux, bool is_tuned_freq, bool from_sdt)
{
	...
	if(is_template(mux) && from_sdt) {
		if(!tune_confirmation.nit_actual_received) { /*when nit actual is received mux_id is known,
			...
			if(!nit_actual_done())
				return nullptr; //we can retry later
		}
	}
	...
	if(is_active_mux) {
		/*NIT: allow overwriting key, but we will only do that if the existing key_src!= SDT_TUNED
			...
			SDT: allow overwriting key
			     do not allow overwriting tuning data
					 ...
					 do not allow overwriting scan_status as we cannot be sure of tuning data
		 */

		preserve = from_sdt ? m::flags((m::MUX_COMMON /* & ~ m::SCAN_STATUS*/ & ~ m::SDT_SI_DATA)|
																	 m::TUNE_DATA | m::MUX_KEY)
		...
		*/
		preserve = m::flags( preserve |  m::TUNE_DATA);
	}

#ifndef NDEBUG
	{
		//check if sat pos has changed
		auto tmp = *chdb::mux_key_ptr(this->dbmux);
		tmp.sat_pos =  mux_key_ptr(mux)->sat_pos;
		assert(!is_active_mux || tmp ==  *mux_key_ptr(mux));
	}
#endif
	if(!this->update_mux(wtxn, mux, now, is_active_mux /*is_reader_mux*/, is_tuned_freq, from_sdt, false /*from_pat*/, preserve))
		return nullptr; //something went wrong, e.g., on wrong sat
	if(is_active_mux || (is_tuned_freq &&  (*mux_key_ptr(mux) == *mux_key_ptr(this->dbmux))))
		active_adapter().on_stream_mux_change(mux);
	...
	if(is_active_mux) {
		...
		if(!((c.key_src == chdb::key_src_t::NONE) ||
				 ...
				 (c.key_src == chdb::key_src_t::PAT_TUNED)
				 )) {
...
*/
mux_data_t* active_si_stream_t::add_fake_nit(db_txn& wtxn)
{
	using namespace chdb;
	namespace m = chdb::update_mux_preserve_t;
	...
	if (expected_sat_pos != sat_pos_none) {
		...
		auto [it, inserted] = nit_data.by_network_id_ts_id.insert_or_assign(std::make_pair(mux_common->network_id, mux_common->ts_id),
...
void active_si_stream_t::check_timeouts() {
	...
	out.format(" tuned_mux={}", this->dbmux);
...
/*
	called when stream is first locked and is a transport stream;
	returns true on success
 */
void active_si_stream_t::init(const chdb::any_mux_t& driver_mux, bool driver_data_reliable) {
	...
	if(!fix_tune_mux_template(driver_mux, driver_data_reliable)) {
		//lock was lost
		this->reset();
		return;
	}
	...
	scan_state.reset_completion_states();
	switch(this->scan_target) {
...
void active_si_stream_t::on_wrong_sat() {
	...
	auto signal_info_ = active_adapter().get_last_signal_info(true /*wait*/);
	this->init(signal_info_->driver_mux, signal_info_->driver_data_reliable); // locked=true in case dish is still moving
	...
	bool on_wrong_sat, bool network_done)
{
	if (network_done) {
		...
		if (network_data.num_muxes == 0 && sdt_actual_done() && tune_confirmation.ts_id_by != confirmed_by_t::NONE) {
			...
			dterrorf("nit_actual does not contain current mux");
  ...
  -updating the unique mux key in case we discover it is wrong
	...
	4. there is data in nit_actual, it agrees with the tuned frequency, but the tuned sat_pos is incorrect
  	-reasons: dish is still moving and we picked up a transient signal, reception from  a nearby sat,
	   failed diseqc switch has brought us to the same sat
...
dtdemux::reset_type_t active_si_stream_t::nit_section_cb_(nit_network_t& network, const subtable_info_t& info)
{
	...
	auto stream_mux = this->dbmux; //the mux whose SI data is being analyzed here
	...
	for (auto& mux : network.muxes) {
		...
		bool is_active_mux = this->matches_reader_mux(mux, false /*from_sdt*/, true /*check_sat_pos*/) && network.is_actual;
		bool is_active_on_other_sat = this->matches_reader_mux(mux, false /*from_sdt*/, false /*check_sat_pos*/) && network.is_actual;
		...
		if (network.is_actual) {
			...
			ret = nit_actual_update_tune_confirmation(mux, is_active_mux, is_active_on_other_sat, is_tuned_freq);
			if (ret != dtdemux::reset_type_t::NO_RESET) {
				wtxn.abort();
				network_data.reset();
				return ret; // definitely on wrong sat
			}
			if(tune_confirmation.on_wrong_sat) {
					auto& dbmux_key = *chdb::mux_key_ptr(this->dbmux);
					auto& mux_key = *chdb::mux_key_ptr(mux);
					if(dbmux_key.sat_pos !=mux_key.sat_pos) {
						dtdebugf("Currenly tuned mux is on other sat req={} got={}; updating", dbmux_key.sat_pos, mux_key.sat_pos);
						dbmux_key.sat_pos = mux_key.sat_pos;
					}
					is_active_mux = true;
...
bool active_si_stream_t::fix_mux(chdb::any_mux_t& mux)
{
	...
	if (can_be_tuned) {
		...
		can_be_tuned = chdb::is_template(mux) ||
...
active_si_stream_t::nit_actual_update_tune_confirmation(chdb::any_mux_t& mux, bool is_active_mux,
	...
	*/
	...
	if (on_wrong_sat) {
		if (is_embedded_si || pat_data.stable_pat()) {
			...
			is_active_mux = is_active_on_other_sat;
			if(is_active_mux) {
				tune_confirmation.on_wrong_sat = std::abs(tuned_mux_key->sat_pos - mux_key->sat_pos) >= sat_pos_tolerance;
				on_wrong_sat = false;
			}
				if (abort_on_wrong_sat())
					ret = dtdemux::reset_type_t::ABORT;
	...
	*/
	...
	if (is_active_mux) {
		...
		*/
		if (tune_confirmation.sat_by == confirmed_by_t::NONE && ! on_wrong_sat) {
...
bool active_si_stream_t::update_mux(
	...
	bool is_reader_mux, bool is_tuned_freq, bool from_sdt, bool from_pat,
	chdb::update_mux_preserve_t::flags preserve) {
	assert(! (from_pat && from_sdt));
	using namespace chdb;
	namespace m = chdb::update_mux_preserve_t;
	const auto& reader_mux = this->dbmux;
	...
	const auto& c =  *mux_common_ptr(reader_mux);
	bool reader_mux_is_scanning = c.scan_status == scan_status_t::ACTIVE;
	bool propagate_scan = reader->tune_options().propagate_scan;
	const auto& scan_id = c.scan_id;
	...
	if((is_tuned_freq || is_reader_mux) && ! is_embedded_si) {
		...
		*/
		if(active_adapter().lock_state.locked_minimal) {
			auto locked = update_reader_mux_parameters_from_frontend(mux);
			auto failed = !locked;
			if(failed) {
	...
	*/
	...
	if(from_sdt) {
		...
		assert(chdb::mux_common_ptr(mux)->tune_src != tune_src_t::TEMPLATE);
...
active_si_stream_t::sdt_process_service(db_txn& wtxn, const chdb::service_t& service, mux_data_t* p_mux_data,
	...
	if (c.is_valid()) {
		...
		if (changed) {
			ch.expired = false;
			if(ch.media_mode != service.media_mode) {
				if (service.media_mode  == chdb::media_mode_t::DATA  && ch.media_mode == chdb::media_mode_t::T2MI
						&& ch.media_mode_from_pmt) {
					//we prefer media_mode from pmt
				} else {
					ch.media_mode = service.media_mode;
					ch.media_mode_from_pmt = false;
				}
			}
...
dtdemux::reset_type_t active_si_stream_t::sdt_section_cb_(txn_proxy_t<chdb::chdb_t> & wtxn,
	...
	for (auto& service : services.services) {
		...
		auto [db_found_, changed] =
			sdt_process_service(wtxn, service, p_mux_data, donotsave, services.is_actual);
		db_found += db_found_;
		db_changed += changed;
	}
	...
	if (done_now) {
		...
		if (!donotsave_stats) {
			...
			if (changed) { //update statistics
				namespace m = chdb::update_mux_preserve_t;
				dtdebugf("Update mux {} tuned={}", mux, this->dbmux);
				...
				bool is_reader_mux = this->matches_reader_mux(mux, true /*from_sdt*/, true /*check_sat_pos*/);
				auto ret = this->update_mux(wtxn, mux, now, is_reader_mux, is_actual /*is_tuned_freq*/,
												 true /*from_sdt*/, false /*from_pat*/, preserve /*preserve*/);
				if(is_actual)
					this->dbmux  = mux;
				if (ret &&(is_reader_mux || is_actual))
					active_adapter().on_stream_mux_change(mux);
			}
		}
		if (is_actual) {
																														...
																														const subtable_info_t& info)
{
	...
	auto wtxn = chdbmgr.wtxn();
	/*
		find out what we already know about network_id, ts_id
...
void active_si_stream_t::init_scanning() {
...
/*
	...
	However, the values in mux have been retrieved from SI data and/or database and may be incorrect
...
bool active_si_stream_t::update_reader_mux_parameters_from_frontend(chdb::any_mux_t& mux) {
	...
	*mux_common_ptr(signal_info.driver_mux) = 	*mux_common_ptr(mux); //overfride things like ts_id
	...
	assert((chdb::mux_common_ptr(mux)->scan_status != chdb::scan_status_t::ACTIVE &&
					...
					chdb::mux_common_ptr(mux)->scan_status != chdb::scan_status_t::RETRY) ||
				 chdb::scan_in_progress(chdb::mux_common_ptr(mux)->scan_id));
	bool driver_data_reliable = signal_info.driver_data_reliable; //true if data comes from neumo drivers, rather than old drivers
	visit_variant(signal_info.driver_mux,
								[&](chdb::dvbs_mux_t& mux) { //mux equals signal_info.driver_mux
									...
									mux.embedding_service_id = p->embedding_service_id;
									...
									if(p->c.tune_src == chdb::tune_src_t::NIT_TUNED) {
										...
										if(!driver_data_reliable || (std::abs((int)mux.frequency - (int) p->frequency) < 50)) {
											...
											use_driver = true;
											dtdebugf("set use_driver tune_src={} driver_data_reliable={}", p->c.tune_src, driver_data_reliable);
										}
										...
										if(!driver_data_reliable || (std::abs((int)mux.symbol_rate - (int) p->symbol_rate) < 10000))
											...
											use_driver = true;
											dtdebugf("set use_driver symbol_rate={}/{} driver_data_reliable={}",
															 mux.symbol_rate, p->symbol_rate, driver_data_reliable);
										}
	...
	dtdebugf("Update driver_mux={} stream_mux={} tune_src={}",
					 signal_info.driver_mux, this->dbmux, (int)chdb::mux_common_ptr(si_mux)->tune_src);
...
reset_type_t active_si_stream_t::pmt_section_cb(const pmt_info_t& pmt, bool isnext) {
	...
	for (const auto& desc : pmt.pid_descriptors) {
		...
		if(is_t2mi) {
			...
			bool encrypted = pmt.is_encrypted();
			if(p.embedding_type == chdb::embedding_type_t::NONE)
				p.embedding_type = encrypted ? chdb::embedding_type_t::ET2MI: chdb::embedding_type_t::T2MI;
			p.embedding_service_id = pmt.service_id;
		}
	}
...
/*
	...
	at least one of them is the "reader" mux (=the mux from which the si table was read)
 */
bool active_si_stream_t::matches_reader_mux(const chdb::any_mux_t& mux, bool from_sdt, bool check_sat_pos)
{
	/*
	...
	*/
	...
	auto stream_mux = this->dbmux;
	return chdb::matches_physical_fuzzy(mux, stream_mux, check_sat_pos);  // true requires that stream_id or t2mi_pid also match
}
...
void active_si_stream_t::update_stream_ids_from_pat(db_txn& wtxn, chdb::any_mux_t& mux)
{
	...
	if(found) {
		...
		auto ret = this->update_mux(wtxn, mux, now, true /*is_active_mux*/, true /*is_tuned_freq*/,
																false /*from_sdt*/, true /*from_pat*/, found ? m::NONE : m::MUX_KEY /*preserve*/);
	...
	for (auto& [service_id, pat_service]: pmt_data.by_service_id) {
		...
		if(pat_service.t2mi_pid != -1) {
			...
			if(dvbs_mux) {
				...
				dvbs_mux->embedding_service_id = pat_service.embedding_service_id;
			}
			...
			auto ret=this->update_mux(wtxn, mux, now, false /*is_reader_mux*/, true /*is_tuned_freq*/,
																false /*from_sdt*/, true /*from_pat*/, preserve);
			if(ret && *chdb::mux_key_ptr(mux) == *chdb::mux_key_ptr(this->dbmux))
				active_adapter().on_stream_mux_change(mux);
