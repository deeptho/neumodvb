...
void wxSVGCanvasCairo::Init(int width, int height, bool alpha) {
	...
	Destroy();
	m_alpha = alpha;
	auto stride = cairo_format_stride_for_width(alpha ? CAIRO_FORMAT_ARGB32 : CAIRO_FORMAT_RGB24, width);
	m_buffer.clear(); //ensure that any old data is set to zero
	m_buffer.resize(stride * height);

	m_surface = cairo_image_surface_create_for_data(&m_buffer[0], alpha ? CAIRO_FORMAT_ARGB32 : CAIRO_FORMAT_RGB24,
																									width, height, stride);
...
unsigned char* wxSVGCanvasCairo::GetBuffer()
{
	return m_buffer.size()>0 ? &m_buffer[0] : nullptr;
}
