...
#if 0
	bool pmt_info_t::es_ca_pid(uint16_t pid) const
	{
		for(auto& desc: pids_descriptors) {
			if (desc.stream_pid == pid)
				return true;
		}
		return false;
	}
...
*/
	...
	inline uint8_t bcdToDec(unsigned char b) { return (uint8_t)(((b >> 4) & 0x0F) * 10 + (b & 0x0F)); }
	...
	void section_parser_t::parse_payload_unit_(bool parse_only_section_header) {
		...
		if (!is_stuffing) {
			if (!parse_only_section_header && hdr.section_syntax_indicator &&
					!crc_is_correct()) { // CA sections may not have a crc!
...
*/
	...
	bool pat_parser_t::parse_pat_section(stored_section_t& section, pat_services_t& pat_services) {
		...
		uint32_t crc UNUSED = section.get<uint32_t>(); // avoid compiler warning
	...
	bool sdt_bat_parser_t::parse_sdt_section(stored_section_t& section, sdt_services_t& ret) {
		...
		while (true) {
			...
			desc_loop_len &= 0xfff;
			if (section.available() < desc_loop_len)
				return false;
			auto end = section.available() - desc_loop_len;
			int tst = section.available();
			while (section.available() > end) {
				auto desc = section.get<descriptor_t>();
				assert(desc.len <= section.available());
				switch (desc.tag) {
				case SI::ServiceDescriptorTag: {
					section.get_fields<service_descriptor_t>(service);
#if 0
				...
				case SI::CaIdentifierDescriptorTag: {
					...
					section.get_fields<ca_identifier_descriptor_t>(service, desc);
				} break;
				case SI::LinkageDescriptorTag:
					section.get_fields<linkage_descriptor_t>(service, desc);
					break;
			...
			if (end > section.available()) {
				dterrorx("Extra bytes after descriptor loop: %d/%d", end, section.available());
				section.skip(end - section.available());
			}
			if (service.service_type == 12 && strcmp(service.name.c_str(), "FreesatHome") == 0)
				ret.has_freesat_home_epg = true;
		}
		if (section.available() < 4) {
			dterrorx("Too few bytes left at end of sdt");
			return false;
		}
		if (section.available() > 4) {
			dterrorx("bytes left at end of sdt");
			section.skip(section.available() - 4);
		}
		uint32_t crc UNUSED = section.get<uint32_t>(); // avoid compiler warning
	...
	bool sdt_bat_parser_t::parse_bat_section(stored_section_t& section, bouquet_t& bouquet) {
...
#endif
		auto bouquet_desc_len = section.get<uint16_t>() & 0xfff;
		if (section.available() < bouquet_desc_len)
			return false;

		auto end = section.available() - bouquet_desc_len;
		int tst = section.available();
		while (section.available() > end) {
			auto desc = section.get<descriptor_t>();
			assert(desc.len <= section.available());
			...
			case SI::BouquetNameDescriptorTag: {
				if (section.get_fields<dvb_text_t>(bouquet.name, desc) < 0) {
					return false;
				}
			} break;
			case SI::CountryAvailabilityDescriptorTag:
			...
			case SI::FSTRegionListDescriptorTag: {
				...
				section.get_fields<fst_region_list_descriptor_t>(region_list, desc);
			} break;
			case SI::FSTChannelCategoryDescriptorTag: {
#if 0
				fst_channel_category_list_t channel_category_list;
				section.get_fields<fst_channel_category_list_descriptor_t>(channel_category_list, desc);
#else
				section.skip(desc.len);
#endif
			} break;
			case SI::FSTCategoryDescriptorTag: {
				...
				section.get_fields<fst_category_description_list_descriptor_t>(channel_category, desc);
#else
				section.skip(desc.len);
#endif
			} break;
	 ...
	 if (section.available() > end)
		...
		if (section.available() < ts_loop_len)
			return false;
		end = section.available() - ts_loop_len;
		tst = section.available();

		while (section.available() > end) {
			if (section.available() < 6)
				return false;
			auto ts_id = section.get<uint16_t>();
			auto network_id = section.get<uint16_t>();
			auto ts_desc_len = section.get<uint16_t>() & 0xfff;
			if (section.available() < ts_desc_len)
				return false;
			auto end1 = section.available() - ts_desc_len;
			auto tst1 = section.available();
			while (section.available() > end1) {
				auto desc = section.get<descriptor_t>();
				assert(desc.len <= section.available());
				switch (desc.tag) {
				...
				case SI::OtvServiceListDescriptorTag: {
					...
					section.get_fields<otv_service_list_descriptor_t>(service_list, desc);
				}
				...
				case SI::ServiceListDescriptorTag: {
					service_list_t service_list{network_id, ts_id, bouquet};
					section.get_fields<service_list_descriptor_t>(service_list, desc);
				} break;
				case SI::FSTServiceListDescriptorTag: {
					fst_service_list_t service_list{network_id, ts_id, fst_preferred_region_id, bouquet};
					section.get_fields<fst_service_list_descriptor_t>(service_list, desc);
				} break;
				...
				if (has_error())
					return false;
				tst1 -= (desc.len + 2);
				assert(tst1 == section.available());
			}
			assert(end1 == section.available());
		}
		assert(end == section.available());
		uint32_t crc UNUSED = section.get<uint32_t>(); // avoid compiler warning
		if (section.available() != 0) {
	...
	bool nit_parser_t::parse_nit_section(stored_section_t& section, nit_network_t& network) {
		...
		desc_loop_len &= 0xfff;
		auto end = section.available() - desc_loop_len;
		auto tst = section.available();
		while (section.available() > end) {
			auto desc = section.get<descriptor_t>();
			switch (desc.tag) {
			case SI::NetworkNameDescriptorTag:
				if (section.get_fields<dvb_text_t>(network.network_name, desc) < 0) {
					return false;
				}
				break;
		...
		assert(section.available() == end);

		// transport stream loop
		desc_loop_len = section.get<uint16_t>();
		desc_loop_len &= 0xfff;
		end = section.available() - desc_loop_len;
		assert(end >= 0);
		while (section.available() > end) {
			bool is_dvbs{false};
			bool is_dvbt{false};
...
#endif
			auto desc_loop_len1 = section.get<uint16_t>() & 0xfff;
			auto end1 = section.available() - desc_loop_len1;
			auto tst1 = section.available();
			while (section.available() > end1) {
				auto desc1 = section.get<descriptor_t>();
				switch (desc1.tag) {
				case SI::FrequencyListDescriptorTag: {
					frequency_list_t frequencies UNUSED;
					section.get_fields<frequency_list_descriptor_t>(frequencies, desc1);
				} break;

				case SI::S2SatelliteDeliverySystemDescriptorTag: {
					dterror("S2SatelliteDeliverySystemDescriptor");
					if (desc1.len > 0) { // solves problem on 5.0W 12340H
						section.get_fields<s2_satellite_delivery_system_descriptor_t>(dvbs_mux);
						if (!is_dvbs) {
				...
				case SI::SatelliteDeliverySystemDescriptorTag: {
					section.get_fields<satellite_delivery_system_descriptor_t>(dvbs_mux);
					dvbs_mux.k.network_id = original_network_id;
					dvbs_mux.k.extra_id = 0; // will be updated in process_nit
					dvbs_mux.k.ts_id = ts_id;
					is_dvbs = true;
				...
				case SI::CableDeliverySystemDescriptorTag: {
					...
					section.get_fields<cable_delivery_system_descriptor_t>(dvbc_mux);
				} break;
				case SI::TerrestrialDeliverySystemDescriptorTag: {
					...
					mux.k.ts_id = ts_id;
					is_dvbt = true;
					if (section.get_fields<terrestrial_delivery_system_descriptor_t>(dvbt_mux) < 0) {
				...
				case SI::ServiceListDescriptorTag: {
					...
					service_list_t service_list{network_id, ts_id, bouquet};
					section.get_fields<service_list_descriptor_t>(service_list, desc1);
				} break;
				case SI::LinkageDescriptorTag: {
					bouquet_linkage_t bouquet_linkage; // todo: this should not be a service
					section.get_fields<linkage_descriptor_t>(bouquet_linkage, desc1);
				} break;
				default:
				case SI::PrivateDataSpecifierDescriptorTag:
				case SI::LogicalChannelDescriptorTag: // could be useful to parse
					section.skip(desc1.len);
					break;
				}
				tst1 -= (desc1.len + 2);
				assert(tst1 == section.available());
		...
		// assert(section.available() == end); //this assertion fails on 52E 11246V because desc_loop_len is 17 instead of 19
	...
	bool eit_parser_t::parse_eit_section(stored_section_t& section, epg_t& ret) {
		...
		auto segment_last_section_number = section.get<uint8_t>();
		auto last_table_id = section.get<uint8_t>();
		while (section.available() > 4) {
			...
			rec.k.event_id = section.get<uint16_t>();

			section.get_fields<start_time_duration_t>(rec);
			...
			while (section.available() > end) {
				...
				case SI::ShortEventDescriptorTag:
					section.get_fields<short_event_descriptor_t>(rec);
					break;
				case SI::ExtendedEventDescriptorTag:
					section.get_fields<extended_event_descriptor_t>(rec);
					break;

				case SI::TimeShiftedEventDescriptorTag:
					section.get_fields<time_shifted_event_descriptor_t>(rec);
					break;

				case SI::ComponentDescriptorTag:
					section.get_fields<component_descriptor_t>(rec, desc);
					break;
				case SI::ContentDescriptorTag:
					section.get_fields<content_descriptor_t>(rec, desc);
					break;
				case SI::MultilingualComponentDescriptorTag:
					section.get_fields<multilingual_component_descriptor_t>(rec, desc);
					break;
				default:
	...
	bool eit_parser_t::parse_sky_section(stored_section_t& section, epg_t& ret) {
		...
		auto mjd_time = section.get<uint16_t>();
		...
		while (section.available() > 4) {
			...
			rec.k.event_id = section.get<uint16_t>(); // byte 10 and 11
			auto desc_loop_len = section.get<uint16_t>();
			desc_loop_len &= 0xfff;
			auto end = section.available() - desc_loop_len;
			auto tst = section.available();
			...
			while (section.available() > end) {
				...
				switch (desc.tag) {
				case 0xb5: // title
					section.get_fields<opentv_title_descriptor_t>(rec, desc, mjd_time);
					break;

				case 0xb9: // summary
					section.get_fields<opentv_summary_descriptor_t>(rec, desc);
					break;
				case 0xbb: // desc
					section.get_fields<opentv_description_descriptor_t>(rec, desc);
					break;
				case 0xc1: // series link
					section.get_fields<opentv_serieslink_descriptor_t>(rec, desc);
					break;
				default:
					section.skip(desc.len);
					break;
	...
	bool pmt_parser_t::parse_pmt_section(stored_section_t& section, pmt_info_t& pmt) {
		...
		if (hdr.current_next) {
			...
			dtdebug("Received a 'next' pmt (ignored)");
			return false;
		}
		current_version_number = hdr.version_number;
		int pid = hdr.pid;
		...
		*/
		...
		pmt.pcr_pid = section.get<uint16_t>() & 0x1fff;
		pmt.pmt_pid = pid;
		...
		while (section.available() > 4) { // 4 is the crc

			auto stream_type = section.get<uint8_t>();
			...
			auto stream_pid = section.get<uint16_t>();
	...
	inline bool operator!=(const ca_info_t& a, const ca_info_t& b) { return !(a == b); }
...
std::ostream& dtdemux::operator<<(std::ostream& os, const dtdemux::pid_info_t& info) {
	...
	for (const auto& s : info.subtitle_descriptors) {
		stdex::printf(os, "PID[%0x] type=0x%x ", s.stream_pid, (int)s.subtitle_type);
		if (s.lang_code[0] != 0)
...
void pmt_parser_t::parse_payload_unit() {
	...
	stored_section_t section(payload);
...
bool mhw2_parser_t::parse_mhw2_channel_section(stored_section_t& section, bouquet_t& bouquet) {
	auto& hdr = *header();
	section.skip(120 - section.bytes_read);
	RETURN_ON_ERROR false;
	auto num_channels = section.get<uint8_t>();
	auto offset = (int)section.bytes_read + (int)(8 * num_channels);
	auto len = (int)section.payload.size() - offset;
	if (len <= 0) {
	...
	data_range_t names((uint8_t*)section.payload.buffer() + offset, len);
	for (int i = 0; i < num_channels; ++i) {
		bouquet_t::chgm_t chgm{};
		auto network_id = section.get<uint16_t>();

		auto ts_id = section.get<uint16_t>();
		auto service_id = section.get<uint16_t>();
		auto unknown UNUSED = section.get<uint16_t>();
...
bool mhw2_parser_t::parse_mhw2_title_section(stored_section_t& section, epg_t& epg) {
	...
	assert(section.payload.buffer()[7] + 1 == channel_id);
	assert(section.bytes_read == 8);
	epg.channel_id = channel_id;
#if 1
	auto x0 = section.get<uint8_t>();
#endif
	while (section.available() >= 20) {
		epg.epg_records.resize(epg.epg_records.size() + 1);
		auto& rec = epg.epg_records[epg.epg_records.size() - 1];
		// bytes 4-6
		auto summary_id1 = section.get<uint16_t>(); // byte 1 and 2
		auto x1 = section.get<uint8_t>();						// byte 3 values: 128 and 255, could be part of summary_id1

		auto summary_id = (uint32_t)section.get<uint8_t>();			 // byte 4
		summary_id = (summary_id << 8) | section.get<uint8_t>(); // byte 5
		summary_id = (summary_id << 8) | section.get<uint8_t>(); // byte 6
		// bytes 7,8,9,10
		auto title_id = section.get<uint32_t>();

		// bytes 11,12,13,14,15,16,17
		section.get_fields<mhw2_start_time_duration_t>(rec); // 7 bytes
		assert(rec.k.start_time > 0);
		// byte 18 and following
		section.get_fields<dvb_text_t>(rec.event_name); // 1 byte

		auto x2 UNUSED = section.get<uint16_t>(); // 2 bytes series link?
...
bool mhw2_parser_t::parse_mhw2_short_summary_section(stored_section_t& section, epg_t& epg) {
	...
	auto x0 = section.get<uint8_t>(); // byte 8
	// byte 9, 10,11

	// Note a summary id?
	uint32_t summary_id = section.get<uint8_t>();
	summary_id = (summary_id << 8) | section.get<uint8_t>();
	summary_id = (summary_id << 8) | section.get<uint8_t>();

	auto x1 = section.get<uint16_t>(); // byte 12,13
	RETURN_ON_ERROR false;
	section.get_fields<dvb_text_t>(rec.story); // 1 byte
	rec.story.sprintf("\n");
	auto x2 = section.get<uint8_t>(); // 1 byte
	RETURN_ON_ERROR false;
	section.get_fields<dvb_text_t>(rec.story);
	section.skip(9);
	RETURN_ON_ERROR false;

	// title id indeed matches
	uint32_t title_id = section.get<uint32_t>();
	rec.k.event_id = summary_id;
...
bool mhw2_parser_t::parse_mhw2_long_summary_section(stored_section_t& section, epg_t& epg) {
	...
	section.get_fields<dvb_text_t>(rec.story); // 1 byte byte pos=19 contains length
	rec.story.sprintf("\n");
	RETURN_ON_ERROR false;													// pos now points here
	section.get_fields<mhw2_dvb_text_t>(rec.story); // 2 bytes length field
...
/*
	...
	At the end of this data, so byte 117+(8*N) there are the channel names, with the 1 byte before each name containing the
	length of the name in the last four bits (byte & 0xF).
