...
class RecordPanel(wx.Panel):
    ...
    def OnFocus(self, event):
        ...
        if past_end:
            ...
            return
        dtdebug(f'CALL set_active panel={self}')
        self.controller.set_active(self)
    ...
    def focus_row(self, last_focused_cell, rowno):
        ...
        dtdebug(f'CALL SetFocus panel={self}')
...
class GridEpgPanel(RecordPanel):
    ...
    def update_info(self, w):
        ...
        f = self.GetFont()
        large = infow.GetFont()
        large.SetPointSize(int(f.GetPointSize()*1))
        infow.Freeze()
...
class LivePanel(wx.Panel):
    ...
    def CmdPlayFaster(self, event=None):
        dtdebug('CmdPlayFaster')
        return wx.GetApp().SetPlayBackSpeed(faster=True)

    def CmdPlaySlower(self, event=None):
        dtdebug('CmdPlaySlower')
        return wx.GetApp().SetPlayBackSpeed(faster=False)
