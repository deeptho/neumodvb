...
static_assert(std::is_trivial<lmdb::val>::value, "lmdb::val must be a POD type");
...
public:
  ...
  template<typename K, typename V>
  bool del(MDB_txn* const txn,
           const K& key, const V& v) {
    const lmdb::val k{&key, sizeof(K)};
    return lmdb::dbi_del(txn, handle(), k, v);
  }
};
...
public:
   ...
   * @throws lmdb::error on failure
   */
  bool get(ss::bytebuffer_& key,
           ss::bytebuffer_& val,
           const MDB_cursor_op op) {
    lmdb::val k{}, v{};
    const bool found = get(k, v, op);
    if (found) {
      key = ss::bytebuffer_::view((uint8_t*)k.data(), k.size(), 0);
			val = ss::bytebuffer_::view((uint8_t*)v.data(), v.size(), 0);
    }
    return found;
  }
   ...
   * @throws lmdb::error on failure
   */
  bool get(ss::string_& key,
           ss::string_& val,
           const MDB_cursor_op op) {
    lmdb::val k{}, v{};
    const bool found = get(k, v, op);
    if (found) {
      key = ss::string_::view(k.data(), k.size(), k.size());
			val = ss::string_::view(v.data(), v.size(), v.size());
    }
    return found;
  }
  ...
  bool find(const ss::bytebuffer_& serialized_key,
            ...
            const MDB_cursor_op op = MDB_SET) {
    lmdb::val k{serialized_key.buffer(), (size_t)serialized_key.size()};
		v_ret = lmdb::val{nullptr, 0};
    return get(k, v_ret, op);
  }

  /**
   * Positions this cursor at the given key and value, only for MDB_DUPSORT
   *
   * @param key
   * @param value
   * @param op
   * @throws lmdb::error on failure
   */
  bool find_both(const ss::bytebuffer_& serialized_key,
								 const ss::bytebuffer_& serialized_val,
								 const MDB_cursor_op op = MDB_GET_BOTH) {
		assert(op == MDB_GET_BOTH || op == MDB_GET_BOTH_RANGE);
    lmdb::val k{serialized_key.buffer(), (size_t)serialized_key.size()};
