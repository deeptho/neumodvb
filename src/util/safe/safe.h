 ...
 * See  https://github.com/LouisCharlesC/safe
 */
...
namespace safe
{
	 ...
	 * @tparam MutexType The type of the mutex.
	 */
	template<bool others_can_write, typename ValueType, typename MutexType = DefaultMutex>
	class thread_public_t
	{
	private:
		/// Type ValueType with reference removed, if present
		using RemoveRefValueType = typename std::remove_reference<ValueType>::type;
		...
		public:
			 ...
			 *
			 * @tparam OtherLockArgs Deduced from otherLockArgs.
			 * @param safe The const thread_public_t object to give protected access to.
			 * @param otherLockArgs Other arguments needed to construct the lock
			 * object.
			 */
			template<typename... OtherLockArgs>
			EXPLICIT_IF_CPP17
			Access(const thread_public_t& safe, OtherLockArgs&&... otherLockArgs):
				Access(safe.m_value, safe.m_mutex.get, std::forward<OtherLockArgs>(otherLockArgs)...)
			{}
			 ...
			 *
			 * @tparam OtherLockArgs Deduced from otherLockArgs.
			 * @param safe The thread_public_t object to give protected access to.
			 * @param otherLockArgs Other arguments needed to construct the lock
			 * object.
			 */
			template<typename... OtherLockArgs>
			EXPLICIT_IF_CPP17
			Access(thread_public_t& safe, OtherLockArgs&&... otherLockArgs):
				Access(safe.m_value, safe.m_mutex.get, std::forward<OtherLockArgs>(otherLockArgs)...)
				{ }
		...
		/// Reference to MutexType.
		using MutexReferenceType = RemoveRefMutexType&;
		inline void check_write_access() {
			assert(others_can_write ||  ::thread_group == thread_group);
		}

	public:


		/// Aliases to ReadAccess and WriteAccess classes for this thread_public_t class.
		template<template<typename> class LockType=DefaultReadOnlyLock>
		using ReadAccess = Access<LockType, AccessMode::ReadOnly>;
		template<template<typename> class LockType=DefaultReadWriteLock>
		using WriteAccess = Access<LockType, AccessMode::ReadWrite>;
		...
		/**
		 * @brief Construct a thread_public_t object
		 */
		thread_public_t() = default;
#if 0
		template<typename... ValueArgs>
		explicit thread_public_t(const char* owner_name, thread_group_t thread_group):
			m_mutex(),
...
#endif
		/**
		 * @brief Construct a thread_public_t object with default construction of
		 * the mutex and perfect forwarding of the other arguments to
		 * construct the value object.
		 *
		 * @tparam ValueArgs Deduced from valueArgs.
		 ...
		 */
		template<typename... ValueArgs>
		explicit thread_public_t(const char* owner_name, thread_group_t thread_group, DefaultConstructMutex, ValueArgs&&... valueArgs):
			m_mutex(),
			m_value(std::forward<ValueArgs>(valueArgs)...),
			owner_name(owner_name),
			thread_group(thread_group)
		...
		/**
		 * @brief Construct a thread_public_t object, forwarding the first
		 * argument to construct the mutex and the other arguments to
		 * construct the value object.
		 *
		 * @tparam MutexArg Deduced from mutexArg.
		...
		template<typename MutexArg, typename... ValueArgs>
		explicit thread_public_t(const char* owner_name, thread_group_t thread_group,
														 MutexArg&& mutexArg, ValueArgs&&... valueArgs):
			m_mutex{std::forward<MutexArg>(mutexArg)},
			m_value(std::forward<ValueArgs>(valueArgs)...),
			owner_name(owner_name),
			thread_group(thread_group)
		...
		/// Use copy(), assign() and other defined constructors to get the behavior
		/// you need with an explicit syntax.
		thread_public_t(const thread_public_t&) = delete;
		thread_public_t(thread_public_t&&) = delete;
		thread_public_t& operator =(const thread_public_t&) = delete;
		thread_public_t& operator =(thread_public_t&&) = delete;
		...
		ConstValueReferenceType owner_read_ref() const noexcept
			{
				assert(!others_can_write);
				return m_value;
		}
